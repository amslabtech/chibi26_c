...
void Astar::obs_expand(const int index)
{
    ...
    // メートル単位で統一（cell_marginを使わない）
    for(int i = x - (int)(margin_ / resolution_) - 1; i <= x + (int)(margin_ / resolution_) + 1; i++){
        for(int j = y - (int)(margin_ / resolution_) - 1; j <= y + (int)(margin_ / resolution_) + 1; j++){
            if(i < 0 || j < 0 || i >= width_ || j >= height_) continue;
            if(new_map_.data[j * width_ + i] == 100) continue;

            // メートル単位で距離判定
            double dx = (i - x) * resolution_;
            double dy = (j - y) * resolution_;
            double dist2 = dx * dx + dy * dy;

            if(dist2 <= margin_ * margin_){
