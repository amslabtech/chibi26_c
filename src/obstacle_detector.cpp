...
: Node("c_obstacle_detector")
{
   // global変数を定義(yamlファイルからパラメータを読み込めるようにすると，パラメータ調整が楽)
    ...
    laser_step_    = this->declare_parameter<int>("laser_step", 5);
    robot_frame_   = this->declare_parameter<std::string>("robot_frame", "base_link");
    ignore_dist_   = this->declare_parameter<double>("ignore_dist", 100.0);
    ignore_pillar_ = this->declare_parameter<double>("ignore_pillar", 0.030);
...
void ObstacleDetector::process()
{
    ...
    if(!scan_obstacle())
    {
        std::cout << "scan_obstacle ERROR" << std::endl;
        return;
    }
}
